...
def recordAudio():
    ...
    try:
        ...
        print("Sorry Boss! I am unable to hear you clearly")
...
def assistant(data):
    ...
    if "how are you" in data:
        say("I am fine Boss, What About You?")
    if "hey Jarvis"in data or "hello Jarvis"in data:
        rand=randint(1,3)
        if rand==1:
            say("I am at your service Boss")
        if rand==2:
            say("Ask me anything")
        if rand==3:
            say("Yes Boss! I am Here")
    ...
    if "check my internet connection"in data or "check internet connection" in data:
        ...
        if response==0:
            ...
            say("Internet Connection is fine Boss")
    ...
    if "take a note" in data:
        ...
        file.write("\n"+' '.join(data))
        file.close()
        say("Note Taken Boss. Any thing Else?")
        data = recordAudio()
        if "no" in data:
            say("Ok.Boss!")
        if "yes" in data:
    ...
    if "where is" in data:
        data = data.split(" ")
        location = data[2]
        say("Just A Second Boss, I will show you where " + location + " is.")
